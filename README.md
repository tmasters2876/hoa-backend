...
Records the Board's decision on every committee proposal — Board Approved (→ community attorney review → community vote), Board Rejected (reason required, shown to the committee), or Deferred — from the **Board Decisions** page (added Sept 2026). The committee can also end a flag itself (Closed by committee, no change needed) without the Board ever seeing it. Also reviews the *tool itself* for accuracy: watches what residents actually ask (Resident Questions), flags and corrects inaccuracies in the clause database that powers the front-end search tool, closes revision flags, and reads the decision history. Includes everything `member` can do.
